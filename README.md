...
Furthermore, you should edit the Username and the GitHub Personal Access Token (classic, scope: read:packages) in `nuget.config` to retrieve the correct JikanDotNet fork package that works around some entries not loading. Should be removed after [the MR](https://github.com/Ervie/jikan.net/pull/69) gets merged and new version is published.
### Code
Spaghetti landfill.
